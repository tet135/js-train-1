...
function isEvenNumber(num) {
  let isEven = (num % 2 === 0) ? true : false // Перевірити, чи число num є парним
  return isEven // Повернути результат перевірки (true або false) як результат функції
}
...
function isPrimeNumber(num) {
  ...
  } else { // В іншому випадку продовжуємо перевірку
...
function checkDataType(value) {
  ...
  return `Значення "${value}" має тип "${dataType}"`;
}
